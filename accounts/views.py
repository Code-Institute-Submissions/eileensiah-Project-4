...
from .forms import Agency_UserCreationForm, Agency_UserChangeForm, UserLoginForm
from accounts.models import Agency_User
from django.contrib.auth import update_session_auth_hash
from django.contrib import auth, messages

class SignUpView(CreateView):
    form_class = Agency_UserCreationForm
...
#     if request.method == "POST":
#         form = Agency_UserCreationForm(request.POST)
#         if form.is_valid():
#             form.save()
...
#             })
#     else:
#         register_form = Agency_UserCreationForm()
#         return render(request, "signup.html", {
#             'form': register_form
#         })
    
def account_edit(request, pk):
    user = get_object_or_404(Agency_User, pk=pk)
    if request.method == "POST":
        form = Agency_UserChangeForm(request.POST, instance=user)
        if form.is_valid():
            user = form.save(commit=False)
            user.save()
            return redirect('account', username=user.username)
    else:
        form = Agency_UserChangeForm(instance=user)
